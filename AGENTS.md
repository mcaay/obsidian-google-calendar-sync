...
- Explicit Google Task row deletion deletes the Google task without a confirmation, including recurring tasks, as requested on 2026-09-19. Deleting all recurring-task occurrences is the desired behavior, but the public Tasks API has no recurrence fields or series-deletion control. Do not claim a whole-series guarantee, pretend to detect recurrence, or add automatic recurrence markers. Late undo creates a one-off task because repeat schedules cannot be restored through this API.
- Row deletions wait five seconds before reaching Google. Native Vim `u` or Cmd+Z within that window cancels deletion; redo starts a fresh window. Preserve pending deadlines across reloads. After the deadline, explicit native undo recreates a Google Task with a new ID, preserving its list, due date, title and completion state. Never recreate Calendar events or recreate tasks from stale file content alone.
